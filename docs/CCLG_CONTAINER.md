...
`cclg.container.v0.1` — **frozen as v1** (2026-07-10). This spec, its
fail-closed load semantics (§3.1.1, §4), and the golden conformance fixtures
under `tests/conformance/` (generated by `scripts/gen_conformance.py`) are now
the normative contract for any implementation of this container format —
Python (this repo, canonical) or otherwise (e.g. the TypeScript port in
`derivatives/schift-ai-memory`). A conformance-fixture-passing implementation
is the bar for calling something ".cclg v1 compatible." This still inherits
`format/cclg.format.v0.1.toml`'s `status = "draft"` `[compatibility]` rule
verbatim: **0.x minor version bumps are breaking.** "Frozen as v1" describes
the *contract's* stability (the invariants in §3, the fail-closed rules in
§3.1.1/§4, the conformance suite), not a claim that `0.1` is the last version
string this format will ever use.
...
#### 3.1.2 Load semantics: an unrecognized patch operation is a hard failure

A patch record's `operation` is drawn from a closed set this reader
classifies exhaustively into "retires its target"
(`patches.RETIRING_PATCH_OPERATIONS`) or "does not retire its target"
(`create`, `rollback`) — together, `patches.KNOWN_PATCH_OPERATIONS`. A
conforming reader MUST NOT silently treat a patch whose `operation` is
outside this closed set as non-retiring by default. Doing so would exactly
reintroduce the bug §3.1.1 exists to close: a future container-format version
(or a hand-edited/corrupted record) could add or carry an operation that
*should* retire its target, and an effective-view computation that quietly
ignores an operation it doesn't recognize would resurrect a memory that was
supposed to be superseded, expired, forgotten, or deprecated.

**`effective_view()` MUST raise on an unrecognized operation** — in the
Python implementation, `patches.UnknownPatchOperationError` — rather than
falling through to "not retiring." This is enforced once, at the
`effective_view()` call site (not in `MemoryPatch.from_dict`, which stays a
plain, non-raising constructor so `schema.py`'s validators can keep
aggregating every problem in a container in one pass rather than failing
fast on the first bad record). In the normal `load_container(..., validate=
True)` path (the default), a patch with an operation outside
`schema.PATCH_OPERATIONS` is already rejected earlier, at schema validation,
with a `ContainerError` aggregating every problem found — `effective_view()`'s
own check is what closes the gap for a `load_container(..., validate=False)`
caller, or any direct `effective_view()`/`ContainerBundle.effective_view()`
call over patches that were never schema-validated at all.
...
- `header.format_id`: cross-check against `CCLG_FORMAT_ID`. **A mismatch is a
  hard `ContainerError` (escalated 2026-07-10; fail-closed load semantics).**
  This spec originally treated `format_id` as informational-only (warn, don't
  hard-fail), on the theory that per-record `schema_version` was the
  authoritative check. That theory has a gap: it silently assumes this
  reader's *other* fixed, closed classifications — most concretely,
  `patches.KNOWN_PATCH_OPERATIONS` (§3.1.2) — still apply to a container
  produced against a different format version, which is exactly the
  assumption a format bump exists to break. A reader that cannot name the
  format it's reading must not guess that its own semantics still hold for
  it. `load_container` implements this by raising `ContainerError` before any
  section or record is otherwise trusted.
...
src/cclg/patches.py
  ...
  KNOWN_PATCH_OPERATIONS: set[str]        # RETIRING_PATCH_OPERATIONS | {"create", "rollback"}
  UnknownPatchOperationError(ValueError)  # raised by effective_view() — see §3.1.2
```
...
- `load_container` accepts an unknown *section* with a warning and still loads
  the known sections (forward compatibility, §7) — but rejects a
  `header.format_id` mismatch with a hard `ContainerError` (§4, escalated
  2026-07-10 — these are not the same category of "more than expected," see
  §4's rationale).
- `effective_view(nodes, patches=patches)` raises
  `patches.UnknownPatchOperationError` for any patch whose `operation` is
  outside `patches.KNOWN_PATCH_OPERATIONS` (§3.1.2) rather than silently
  treating it as non-retiring.
...
Golden `.cclg` fixtures + their expected effective-view JSON live in
`tests/conformance/` (generated deterministically by
`scripts/gen_conformance.py` — fixed ids/timestamps, no wall-clock
non-determinism, so regenerating produces byte-identical output). They are
committed artifacts, not generated at test time, and cover:
...
6. rollback
7. a container carrying an unknown patch operation — a **negative** fixture:
   loading it and computing the effective view MUST raise
   (`patches.UnknownPatchOperationError`)
8. a container carrying a forbidden auth field — a negative fixture verifying
   the existing §3.2 auth-free guard (`ContainerError`)

`tests/test_conformance.py` loads each fixture, computes
`ContainerBundle.effective_view()`, and compares against the fixture's
expected-view JSON (sorted keys, id lists — language-neutral on purpose).
Any implementation of this spec, in any language, MUST reproduce the same
expected views (or the same raised-error behavior for the negative fixtures)
over these same fixture files to be considered conformant — this is the
mechanism that keeps a second implementation (e.g. the TypeScript port in
`derivatives/schift-ai-memory`) from silently drifting from
`patches.RETIRING_PATCH_OPERATIONS` / `patches.KNOWN_PATCH_OPERATIONS`.
